...
public class XWingStrategy : BaseStrategy<XWingStrategy>
{
    ...
    public override IPuzzleAction? Execute(Grid grid)
    {
        ...
        FindXWings(grid.Rows, grid.Columns,UnitType.Row, UnitType.Column, action); // Look for x-wing in the rows
        FindXWings(grid.Columns, grid.Rows,UnitType.Column, UnitType.Row, action); // Look for x-wing in the columns
    ...
    private void FindXWings(Unit[] base_sets, Unit[] cover_sets, UnitType base_set_type, UnitType cover_set_type, EliminationSolvePuzzleAction action)
    {
        ...
        for (int i = 0; i < candidates.Count - 1; i++)
        {
            for (int j = i + 1; j < candidates.Count; j++)
            {
                if (IsXWing(candidates[i], candidates[j]))
                {
                    ...
                    EliminateCandidates(value, units_to_process, indices_to_ignore, base_set_type, action);
                }
            }
        }
    }

    private void EliminateCandidates(int value, Unit[] cover_sets, int[] indices_to_ignore, UnitType base_set_type, EliminationSolvePuzzleAction action)
    {
        ...
        {
            var cells_to_eliminate_value_in = cover_unit.Cells.Where(c => !indices_to_ignore.Contains(c.GetIndexInUnit(base_set_type)) && c.Has(value));

            if (cells_to_eliminate_value_in.Any())
                action.Add(new SolveActionElement
                {
                    Description = $"An x-wing on {value} (in {base_set_type} and cells {string.Join(',',xwing_cells.Select(c => c.Index))}) removes {value} from {cover_unit.FullName} (cells {string.Join(',', cells_to_eliminate_value_in.Select(c => c.Index))})",
                    Number = value,
