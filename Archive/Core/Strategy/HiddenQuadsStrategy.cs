...
public class HiddenQuadsStrategy : BaseStrategy<HiddenQuadsStrategy>
{
    ...
    public override IPuzzleAction? Execute(Grid grid)
    {
        ...
        foreach (var unit in grid.AllUnits)
            FindHiddenQuads(unit, action, quads_found);
    ...
    private void FindHiddenQuads(Unit unit, EliminationSolvePuzzleAction action, HashSet<(Cell, Cell, Cell, Cell)> quads_found)
    {
                    ...
                    for (var m = k + 1; m <= 9; m++)
                    {
                            ...
                            digit_to_cells.ContainsKey(m))
                        {
                            ...
                            if (union.Length == 4)
                            {
                                ...
                                {
                                    foreach (var candidates_to_eliminate in cell.Candidates.Except([i, j, k, m]))
                                        action.Add(new SolveActionElement()
                                        {
                                            Description = $"Hidden quad ({i},{j},{k},{m}) in {unit.FullName}: Eliminates {candidates_to_eliminate} in cell {cell.Index}",
