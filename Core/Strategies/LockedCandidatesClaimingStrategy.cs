    ...
    public override ICommand? Plan(Grid grid)
    {
        ...
        foreach (var box in grid.Boxes)
        {
            foreach (var unit in units)
                FindLockedCandidates(grid, box, unit, command);
        }

        return command.IsValid() ? command : null;
    }

    private void FindLockedCandidates(Grid grid, Unit box, Unit unit, LockedCandidatesClaimingCommand command)
    {
        ...
        if (values_only_in_box.Length > 0)
        {
            foreach (var value in values_only_in_box)
            {
                ...
                var cells_to_update = box.Cells.Except(unit.Cells).Where(c => c.IsEmpty && c.Contains(value)).ToList();
                var claiming_cells = box.Cells.Where(c => c.IsEmpty && c.Contains(value)).Except(cells_to_update).ToList();
                ...
                if (cells_to_update.Count > 0)
                    command.Add(new CommandElement
                    {
                        ...
                        CellsToVisualize = claiming_cells
                    });
            }
        }
    }
