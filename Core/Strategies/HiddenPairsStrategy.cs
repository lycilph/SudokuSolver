...
public class HiddenPairsStrategy : BaseStrategy<HiddenPairsStrategy>
{
    ...
    public override ICommand? Plan(Grid grid)
    {
        ...
        foreach (var unit in grid.AllUnits)
            FindHiddenPairs(unit, command, pairs_found);

        return command.IsValid() ? command : null;
    }

    private void FindHiddenPairs(Unit unit, HiddenPairsCommand command, HashSet<(Cell, Cell)> pairs_found)
    {
            ...
            for (int j = i + 1; j <= 9; j++)
            {
                    ...
                    digit_to_cells[i].SequenceEqual(digit_to_cells[j]))
                {
                    ...
                    if (!pairs_found.Contains(cell_pair))
                    {
                        ...
                        {
                            foreach (var candidate_to_eliminate in cell.Candidates.Except([i, j]))
