...
public class ImageImporter
{
    ...
    private List<CellImage> cells = [];
    private List<CellDigit> digits = [];
    ...
    {
        var chars = digits.Select(d => string.IsNullOrWhiteSpace(d.Digit) ? "." : d.Digit).ToArray();
    ...
    public void ExtractCells(CellsExtractionParameters parameters)
    {
        ...
        for (int i = 0; i < contours.Size; i++)
        {
                ...
                aspect_ratio > 0.6 && aspect_ratio < 1.4)
            {
                ...
                cells.Add(new CellImage { Image = grid_image.Copy(rect), Center = pt });
            }
    ...
    public void ExtractDigits()
    {
        ...
        var digits_found = digits.Count(d => !string.IsNullOrEmpty(d.Digit));
        var failures = digits.Count(d => d.RecognitionFailure);

        sb.AppendLine($"Found {digits_found} digits ({failures} failures)");
    }

    public CellDigit ExtractDigit(CellImage cell, DigitExtractionParameters parameters)
    {
        sb.AppendLine($"* Extracting digit from cell {cell.Id} *");

        var digit = new CellDigit(cell);
    ...
    {
        var cells_to_cleanup = digits.Where(d => d.RecognitionFailure || (!string.IsNullOrWhiteSpace(d.Digit) && d.Confidence < 50.0)).ToList();

        foreach (var cell in cells_to_cleanup)
        {
            var alternatives = DigitExtractionParameters
                .Select(p => ExtractDigit(cell.CellImage, p))
                .ToList();
